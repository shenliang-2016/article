...
> 这是一个高级主题，如果你只是 AOP 的初学者，可以安全地跳过本章节。

默认情况下，应用上下文中每个切面只存在一个实例。AspectJ 称之为单例实例化模型。以备用生命周期定义切面是可能的。Spring 支持 AspectJ 的`perthis`和`pertarget`实例化模型（`percflow`，`percflowbelow`，以及`pertypewithin`当前不支持）。

你可以通过在`@Aspect`注解中指定一个`perthis`子句来声明一个`perthis`切面。考虑下面的例子：
...
在前面的示例中，`perthis`子句的作用是为执行业务服务的每个唯一服务对象创建一个切面实例（每个唯一对象在由切入点表达式匹配的连接点处绑定到`this`）。切面实例是在第一次在服务对象上调用方法时创建的。当服务对象退出作用域时，该切面也退出作用域。在创建切面实例之前，其中没有任何增强执行。一旦创建了切面实例，在其中声明的增强就会在匹配的连接点执行，但仅在服务对象就是与此方面关联的服务对象时执行。有关`per`子句的更多信息，请参阅 AspectJ 编程指南。

`pertarget`实例化模型与`perthis`模型工作方式完全一样，不过它会为匹配的连接点上的每个唯一目标对象创建一个切面实例。

#### 5.4.7 一个 AOP 例子

现在你已经了解了所有部分的工作原理，现在我们把它们组合在一起做点有用的事情。

业务服务的执行有时候会由于并发问题（比如死锁）而失败。如果一个操作被重试，它就可能会在下一次尝试时成功。对对那些在这种情况下适合重试的业务服务（无需将结果返回给用户以进行冲突解决的幂等操作），我们希望可以透明重试而避免让客户端看到`PessimisticLockingFailureException`。这是一个清晰的横切服务层中的多个服务的需求，因此，通过切面实现是个理想的方案。

因为我们想要重试操作，我们需要使用环绕增强以便可以多次调用`proceed`。下面的例子展示了基本的切面实现：
...
注意，切面实现了`Ordered`接口，因此我们可以将此切面的优先级设置为高于事务增强（我们希望每次重试都有一个新的事务）。`maxRetries`和`order`属性都由 Spring 配置。主要的操作都发生在`doConcurrentOperation`环绕增强中。注意，此时，我们将重试逻辑应用于每个`businessService()`。我们尝试执行业务逻辑，如果由于`PessimisticLockingFailureException`，我们重试，除非我们耗尽所有的重试次数。
...
为了优化切面以便它只会为幂等操作执行重试，我们可以定义如下的`Idempotent`注解：
...
然后我们就可以使用该注解修饰服务操作的实现。将切面修改为仅仅对幂等操作进行重试涉及到优化切入点表达式为仅仅匹配`@Idempotent`操作。如下所示：
