...
此例子配置创建了一个名为`limitbyaddr`的共享存储区域。使用的预定义的键是客户端的 IP 地址的二进制形式。该共享存储区的大小设定为 10 MB。该区域使用关键字参数设定比率。`limit_req`指令携带两个可选的关键词参数：`zone`和`burst`。`zone`用来指示指令使用哪个共享存储区域来限制请求。当一个给定区域的请求比率达到阈值时，请求就会被延迟，直到达到它们的最大崩溃阈值，该阈值由`burst`关键字参数确定。`burst`关键字参数默认是`0`。`limit_req`同时携带了第三个可选参数，`nodelay`。这个参数允许客户端在被限制之前无延迟地使用它的`burst`。`limit_req_status`设定返回给客户端的状态为一个特定的 HTTP 状态码，默认是`503`。`limit_req_status`和`limit_req`在 HTTP、服务器以及位置上下文中都可用。`limit_req_zone`仅在 HTTP 上下文中可用。NGINX Plus v R16 中的比率限制是集群敏感的。

### 讨论



NGINX Plus, new in version R16.