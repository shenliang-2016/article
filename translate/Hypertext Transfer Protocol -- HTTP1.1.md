...
此文档为因特网通信指定了一个标准追踪协议，同时希望与大家讨论并得到建议以不断改进。请将 “因特网官方协议标准” 的当前版本作为本协议的标准化状态。任何人都可以不受限制地分发此文档。

## 摘要

超文本传输协议 HTTP 是一个应用层协议，用于分布式、协作的、异构的信息系统。它是一个通用的、无状态的协议。通过扩展其请求方法，错误码和协议首部，还可以完成超文本传输之外的更多其它任务。比如命名服务器和分布式对象管理系统等。HTTP 的一大特征就是类型和数据表示协商，这将允许相互传递数据的系统可以互不依赖地创建。
...
````Date````通用首部字段表示该消息产生的时间和日期，具有与 RFC 822 中````orig-date````相同的语法。该字段的值时一个````HTTP-date````，如 3.3.1 章节中所述，它必须被写入 RFC 1123 日期格式。
...
初始服务器必须在所有响应中包含````Date````首部字段，除非：

1. 如果响应状态码是 100（Continue）或者 101（Switching Protocols），该响应可以包含一个````Date````首部字段，这是服务器的选项。
2. 如果响应状态码表示一个服务器错误，比如 500（Internal Server Error）或者 503（Service Unavailable），因而产生一个合法的````Date````非常不方便甚至不可能。
3. 如果服务器没有时钟可以提供可靠的当前时间，它的响应就绝对不能包含````Date````首部字段。这种情况下，必须遵循 14.18.1 章节中的规则。

接收到的消息，如果不包含````Date````首部字段，接收方就必须设法为其添加上，如果该消息将被该接收方缓存或者将通过一个需要````Date````首部字段的代理服务器进行转发。一个没有时钟的 HTTP 实现绝对不能缓存响应，而在每次使用它们之前不进行再验证。一个 HTTP 缓存服务器，特别是共享缓存，应该使用一种机制，比如 NTP，与可靠的外部标准时钟保持同步时间。

客户端应该仅仅再包含实体主体的消息中发送````Date````首部字段，如同````PUT````和````POST````请求的情况，即便如此，它仍然是可选的。没有时钟的客户端绝对不能在请求中发送````Date````首部字段。

在````Date````首部字段中发送的 HTTP-date 不应该表示消息产生之后的时间和日期。它应该表示消息产生的最佳可用的时间和日期估计值，除非该实现没有产生精确时间日期的能力。理论上，该日期时间应该表示数据实体产生之前的那一时刻。在实践中，该时间可以在消息产生过程中的任何时间产生，而不会影响其语义值。

### 14.18.1 无时钟的初始服务器操作

一些初始服务器实现可能没有可用的时钟。没有时钟的初始服务器绝对不能在响应中包含````Expires````或者````Last-Modified````值，除非这些值相关的资源由拥有可靠时钟的系统或者用户提供。它可以分配一个众所周知的````Expires````值，在服务器配置时刻或者之前，在过去（这就允许响应的"预超时"而不为每个资源单独存储````Expires````值）。

## 14.19 ETag

````ETag````响应首部字段为请求的变量提供了实体标签的当前值。与实体标签共同使用的首部字段在 14.24、14.26 和 14.44 章节中描述。实体标签可以被用来与来自相同资源的其它实体相比较。
...
````Expires````实体首部字段给出了一个时间点，在该时间点之后该响应就会被认为是过时的。一个过时的缓存实体通常不能被缓存（代理服务器上的缓存或者用户代理上的缓存）返回，除非首先与初始服务器进行再次验证（或者与拥有该实体的新鲜副本的中间缓存）。参考 13.2 章节了解更多超时模型细节。

````Expires````首部字段的存在并不意味着初始资源将会在该时间点上、之前或者之后改变或者消失。

该字段内容是 3.3.1 章节中描述的````HTTP-date````定义的绝对日期和时间格式。必须是 RFC 1123 日期格式：
...
注意：如果一个响应包含一个````Cache-Control````字段，携带````max-age````指令，该指令将覆盖````Expires````字段。

HTTP/1.1 客户端和缓存必须对待其它不合法的日期格式，特别是其中包含的“0”值，意味着过去，已经超时失效。

为了标识一个响应为“已经超时失效”的，初始服务器发送一个````Expires````日期等于````Date````首部字段。（参考 13.2.4 章节中介绍的超时计算规则）

为了标识一个响应为“永不超时失效”的，源服务器发送一个````Expires````日期大约晚于响应发送时刻一年。HTTP/1.1 服务器将来也不应该发送任何超过一年的````Expires````日期。

响应中值为未来某个时间点的````Expires````首部字段的存在可以将默认为不可缓存的响应指定为可缓存的，除非````Cache-Control````首部字段另有指示。

## 14.22 From

````From````请求首部字段，如果给定，应该包含一个因特网电子邮件地址，属于控制发起请求的用户代理的人类用户。该地址应该是机器可用的，如同在 RFC 822 中定义并在 RFC 1123 中升级的"mailbox"。
...
此首部字段可以被用于日志目的或者用来确定非法的或者不希望的请求的来源。它不应该被用于不安全形式的访问保护。该字段的含义是该请求是给定的人执行的，该人员对所执行的方法负责。 特别地，机器人用户代理应该在请求中携带这个首部字段，以便在接收端出现问题时可以联系负责运行该机器人的人员。

此字段中的因特网电子邮件地址可以独立于发出请求的因特网主机。比如，当请求穿过了代理服务器，则源用户的地址就应该被使用。

没有经过用户的确认，客户端不应该发送````From````首部字段，因为它可能不符合用户的隐私倾向或者站点的安全策略。强烈推荐确保用户可以在请求发送之前的任何时刻关闭、开启并且修改此字段的值。

## 14.23 Host

````Host````请求首部字段指定了被请求的资源所在的因特网主机和端口号，如同从用户给出的或者指向请求资源的初始 URL 中获取的那样。````Host````字段值必须表示初始 URL 给出的初始服务器或者网关服务器的权威命名。这就允许初始服务器或者网关服务器区别内部不明确的 URLs，比如，单个 IP 地址上的多个主机名的服务器的根 URL "/"。

````
Host = "Host" ":" host [ ":" port ] ; Section 3.2.2
````

不带端口号的主机地址暗示请求服务的默认端口（比如，对 HTTP URL 来说就是 80 端口）。比如，一个初始服务器上的指向````<http://www.w3.org/pub/WWW/>````的请求可能包含：
...
客户端必须在所有 HTTP/1.1 请求消息中包含````Host````首部字段。如果请求的 URI 不包含一个被请求服务的因特网主机名，则````Host````首部字段必须被指定为空值。一个 HTTP/1.1 代理服务器必须确保它转发的所有请求消息都包含一个合适的````Host````首部字段来指明请求的服务。所有基于因特网的 HTTP/1.1 服务器都必须响应 400 (Bad Request) 状态码给任何缺少````Host````首部字段的 HTTP/1.1 请求消息。

## 14.24 If-Match

````If-Match````请求首部字段用在方法上表示该方法是条件式的。拥有从某个资源获取的一个或者多个实体的客户端可以通过在````If-Match````首部字段中包含与这些实体关联的实体标签的方式来验证它们是否仍然是新鲜有效的。尸体标签在 3.11 章节中定义。此特性的目的是允许以最小的事务开销有效地更新缓存信息。它同时也被用于更新请求，来防止无意中修改错误版本的资源。作为特殊情况，值“*”匹配到资源的任何当前实体。

````
If-Match = "If-Match" ":" ( "*" | 1#entity-tag )
````

如果任何实体标签与在该资源上对类似 GET 请求（没有 If-Match 首部字段）的响应中返回的实体的实体标签匹配，或者如果给出“*”并且存在任何当前实体对应于该资源，则服务器可以执行所请求的方法，就好像 If-Match 首部字段不存在一样。

服务器必须使用强比较函数来比较````If-Match````中的实体标签。

如果没有匹配的实体标签，或者给定了"*"但是并没有实体存在，服务器绝对不能执行该请求的方法，同时必须返回一个 412 (Precondition Failed) 状态码响应。这种行为最有用的场景是当客户端想要防止更新方法，比如 PUT，修改自从客户端上次获取以来已经发生变化的资源。

如果请求没有携带````If-Match````首部字段的情况下会导致除了 2xx 或者 412 状态码之外的任何其他状态码响应，则````If-Match````首部字段必须被忽略。

"If-Match: *" 的含义是该方法应该被执行，如果初始服务器（或者由缓存服务器选择的，可能使用````Vary````机制，参考 14.44 章节）选择的表现形式存在，如果该表现形式不存在则绝对不能执行。

视图更新一个资源的请求可以包含一个````If-Match````首部字段来表示该请求方法绝对不能被执行，如果对应于````If-Match````值（一个单个的实体标签）的实体不再是该资源的表达。这就允许用户声明他们不希望请求被成功执行，如果该资源已经在他们不知情的情况下发生了变化。比如：
...
同时包含一个````If-Match````首部字段和````If-None-Match````或者````If-Modified-Since````首部字段的请求的结果本规范并未定义。

## 14.25 If-Modified-Since

````If-Modified-Since````请求首部字段与请求方法共同使用来将其标记为条件式的；如果该请求的变体在该字段表示的时间点之后没有发生变化，数据实体就不会从服务器返回。否则，一个不包含消息体的 304 (not modified) 状态码响应就会被返回。
...
此特性的目的是允许高效地更新缓存信息，而只需要最小化的事务开销。

注意：````Range```` 请求首部字段改变了 ````If-Modified-Since```` 首部字段的含义。参考 14.35 章节获取完整细节。

注意：````If-Modified-Since```` 时间由服务器解读，而该服务器的时钟可能并不与客户端同步。

注意：在处理 ````If-Modified-Since```` 首部字段时，一些服务器将使用精确的日期相等函数，而不是小于函数，来确定是否应该发送 304（Not Modified）状态码响应。为了获取最佳结果，在为缓存验证发送一个 ````if-Modified-Since```` 首部字段时，建议客户端尽可能使用前一个 ````Last-Modified```` 首部字段中接收到的精确日期字符串。

注意：如果客户端使用一个任意时间在 ````If-Modified-Since```` 首部字段中，而没有使用同一个请求中的 ````Last-Modified```` 首部字段中的日期值，该客户端应该意识到该时间值直接影响服务器对时间的理解。该客户端应该考虑非同步时钟以及由于客户端和服务器的时间编码不同而导致的相关问题。这就包含了出现竞争条件的可能性，如果文档的修改发生在它首次被请求之后，同时又在后续请求的 ````If-Modified-Since```` 时间之前，则可能就会出现时钟漂移相关问题，如果产生自客户端时钟的 ````If-Modified-Since```` 无法对准服务器时钟。由于网络误差的存在，客户端和服务器之间不同时间基点的对准永远只可能是近似的。

同时携带 ````If-Modified-Since```` 首部字段和 ````If-Match```` 首部字段或者 ````if-Unmodified-Since```` 首部字段的请求结果本规范并未定义。

## 14.26 If-None-Match

````If-None-Match```` 请求首部字段与请求方法共同使用来将其标记为条件式的。先前从资源获取到一个或者多个数据实体的客户端能够校验那些实体是否新鲜，通过将它们先关联的实体标签列表放在 ````If-None-Match```` 首部字段中。此特性的目的是允许高效地更新缓存信息，而只需要最小化的事务开销。它也被用来防止意外的请求方法修改已经存在的资源，当客户端相信该资源并不存在时。

作为特殊情况，值 “*” 匹配资源的任何当前数据实体。

````
If-None-Match = "If-None-Match" ":" ( "*" | 1#entity-tag )
````

如果列表中的任何实体标签匹配到实体的实体标签，该实体可能已经被放在响应中被返回，类似于一个对该资源的 GET 请求（不携带 ````If-None-Match```` 首部字段），或者，如果该字段值为 “*” 而存在该资源的任何新鲜的数据实体，则服务器绝对不能执行该请求方法，除非由于该资源的修改时间与该请求携带的 ````If-Modified-Since```` 首部字段中给出的时间不同而必须执行该请求方法。另一方面，如果请求方法是 GET 或者 HEAD，服务器应该响应 304 （Not Modified）状态码响应，响应中匹配到的一个数据实体携带缓存相关首部字段（特别是 ETag）。对所有其它请求方法，服务器必须响应以 412（Precondition Failed）状态码。

参考 13.3.3 章节了解确定两个实体标签是否匹配的规则。该弱比较函数只能被用于 GET 或者 HEAD 请求。

如果没有任何实体标签匹配，则服务器可以执行请求方法，仿佛 ````If-None-Match```` 首部字段不存在，但是必须同时忽略请求中的所有 ````If-Modified-Since```` 首部字段。也就是说，如果没有实体标签匹配，则服务器绝对不能返回 304 (Not Modified) 状态码响应。

如果请求在没有 ````If-None-Match```` 首部字段的情况下会产生除 2xx 或 304 状态码之外的任何内容，则必须忽略 ````If-None-Match```` 首部字段。（当同一请求中出现 ````If-Modified-Since```` 和 ````If-None-Match```` 时，有关服务器行为的讨论，请参阅第13.3.4节。）

````If-None-Match: *```` 的含义是该方法绝对不能被执行，如果初始服务器（或由缓存服务器，可能使用 ````Vary```` 机制，参考 14.44 章节）选择的表示形式存在，否则就应该执行。此特性是为了防止 PUT 操作之间的竞争。
...
同时携带 ````If-None-Match```` 首部字段和 ````If-Match```` 首部字段或者 ````if-Unmodified-Since```` 首部字段的请求结果本规范并未定义。

## 14.27 If-Range

如果客户端在其缓存中持有实体的部分副本，并且希望将整个实体的最新副本更新到其缓存中，则它可以使用条件 GET 请求，携带 ````Range```` 请求头（使用 ````If-Unmodified- Since```` 和 ````If-Match```` 其中一个或两者）。但是，如果条件因实体已被修改而失败，则客户端将必须发出第二个请求以获取整个当前实体。

````If-Range```` 首部字段允许客户端 "短路" 第二个请求。非正式地，它的含义是：如果数据实体没有改变，将我缺少的部分数据发送给我，否则，将完整的新的数据实体发送给我。

````
If-Range = "If-Range" ":" ( entiry-tag | HTTP-date )
````

如果客户端没有实体标签对应于一个实体，但是拥有一个 ````Last-Modified```` 时间，它可以使用该时间在一个 ````If-Range```` 首部字段中。（服务器能够区分一个合法的 ````HTTP-date```` 与任何形式的 ````entity-tag```` 通过检查不超过两个字符。）````If-Range```` 首部字段应该仅仅与 ````Range```` 首部字段共同使用，同时，如果请求中没有包含 ````Range```` 首部字段，或者服务器不支持子范围操作，则 ````If-Range```` 首部字段就必须被忽略。

如果 ````If-Range```` 首部字段中给出的实体标签与实体的当前实体标签匹配，那么服务器应该使用 206（部分内容）响应提供实体的指定子范围。如果实体标签不匹配，那么服务器应该使用 200（OK）响应返回整个实体。

## 14.28 If-Unmodified-Since

````If-Unmodified-Since```` 请求首部字段被用于请求方法来将其标记为条件式的。如果请求的资源自从该字段指定的时间点以来没有被修改，则服务器应该执行请求的操作，就如同 ````If-Unmodified-Since```` 首部字段不存在一样。

如果请求的变体在给定的时间以来已经被修改，服务器就绝对不能执行请求操作，而且必须返回一个 412 (Precondition Failed) 状态码响应。
...
如果请求正常情况下（比如，不包含 ````If-Unmodified-Since```` 首部字段）将导致 2xx 或者 412 之外的状态码响应，则 ````If-Unmodified-Since```` 首部字段应该被忽略。

如果指定的时间是非法的，则该首部字段应该被忽略。

同时包含 ````If-Unmodified-Since```` 首部字段和 ````If-None-Match```` 或者 ````If-Modified-Since```` 首部字段的请求的结果本规范没有定义。

## 14.29 Last-Modified

````Last-Modified```` 实体首部字段表示初始服务器相信的变体最后被修改的日期和时间。
...
此首部字段的确切含义取决于初始服务器的实现和原始资源的自然特性。对文件来说，它可以是文件系统中的最后修改时间。对包含动态内容的数据实体来说，它可以是它的组成部分集合的最后修改时间。对数据库网关来说，它可以是记录的最后更新时间戳。对虚拟对象来说，它可以是其内部状态变化的最后时间。

原始服务器不得发送比服务器发送消息的时间晚的 ````Last-Modified```` 时间。在这种情况下，资源的最后修改将指示将来的某个时间，服务器必须用消息发起时间替换该时间。
原始服务器应该尽可能使实体的 ````Last-Modified```` 值接近生成该响应的 ````Date````值的时间。这允许接收者准确评估实体的修改时间，特别是如果实体在生成响应的时间点附近发生变化。
HTTP/1.1 服务器应该尽可能发送 ````Last-Modified````。

## 14.30 Location

````Location```` 响应首部字段被用来将消息接收者重定向到不同于原 ````Request-URI```` 以外的位置以完成请求处理，或者标示一个新的资源。对 201 (Created) 状态码响应来说，````Location```` 就是被该请求创建的新资源的位置。对 3xx 状态码响应来说，````Location```` 首部字段应该表明服务器自动重定向请求道资源的首选 URI 。该字段值由一个单独的绝对 URI 组成。 
...
注意：````Content-Location```` 首部字段不同于 ````Location```` ，````Content-Location```` 字段标示请求中包含的数据实体的初始位置。响应中同时包含 ````Location```` 和 ````Content-Location```` 首部字段是可能的。参考 13.10 章节了解某些方法的缓存需求。

## 14.31 Max-Forwards

````Max-Forwards```` 请求首部字段提供了一种机制，为 TRACE 和 OPTIONS 方法限制能够将该请求转发给上游服务器的代理或者网关的数量。当客户端试图跟踪一个在途中发生失败或者环路的请求链时非常有用。

````
Max-Forwards = "Max-Forwards" ":" 1*DIGIT
````

````Max-Forwards```` 值是一个十进制整数，表示该请求消息剩余的可被转发的次数。

每个代理或者网关，作为包含 ````Max-Forwards```` 首部字段的 TRACE 或者 OPTIONS 请求的接收者，在转发该请求之前必须检查并更新它的值。如果接收到的值是 0 ，则接收者绝对不能转发该请求，相反，它必须作为最终的接收者给予响应。如果接收到的 ````Max-Forwards```` 值大于 0 ，则被转发的消息必须包含更新之后的 ````Max-Forwards```` ，该值被减去 1 。

对于本规范定义的所有其他方法以及未明确将其称为该方法定义的一部分的任何扩展方法，将忽略 ````Max-Forwards```` 首部字段。
...
使用条件或无条件 GET 方法的 HTTP 检索请求可以使用 `Range` 请求首部字段请求实体的一个或多个子范围，而不是整个实体，该请求首部字段适用于作为请求结果返回的实体：

````
Range = "Range" ":" ranges-specifier
````

服务器可以忽略 `Range` 首部字段。不过 HTTP/1.1 初始服务器和中间缓存服务器应该尽可能支持字节范围请求，因为 `Range` 支持从部分传输失败中高效恢复，也支持高效获取巨大数据实体的部分数据。

如果服务器支持 `Range` 首部字段，同时指定的范围对数据实体是合适的：

* 如果 GET 成功，则无条件 GET 中存在的 `Range` 首部字段会修改返回的内容。换句话说，响应携带状态代码206（Partial Content）而不是200（OK）。
* 条件 GET 中存在 `Range` 首部字段（使用 `If-ModifiedSince` 和 `If-None-Match` 中的一个或两个的请求，或者 `If-Unmodified-Since` 和 `If-Match` 中的一个或两个）修改 GET 时返回的内容，如果请求成功并且条件是真的。如果条件为 `false`，则不会影响返回的 304（Not Modified）响应。

在某些情况下，除了 `Range` 首部字段之外，使用 `If-Range` 首部字段（参见第14.27节）可能更合适。
...
`TE` 请求首部字段表示希望在响应中接受的扩展传输编码，以及它是否希望在分片传输编码中接受尾部字段。它的值可能由关键字 `trailers` 和/或一个逗号分隔的扩展传输编码名称列表，携带可选的接受参数（如 3.6 章节中所述）。
...
关键字`trailers` 的存在表示客户端希望接受分片传输编码中的尾部字段，如 3.6.1 章节中所述。此关键字被保留用来作为传输编码值使用，尽管它本身并不表示传输编码。
...
`TE` 首部字段仅仅用于立即连接。因此，只要`TE` 出现在 HTTP/1.1 消息中，该关键字必须在`Connection` 首部字段内部使用。

服务器根据 `TE` 字段来判断传输编码是否可接受，按照以下规则：

1. `chunked` 传输编码总是可以接受的。如果列出关键字`trailers`，则客户端表示它愿意代表自己和任何下游客户端接受分块响应中的`trailer`字段。这意味着，如果给出，客户端会声明所有下游客户端愿意接受转发响应中的`trailer`字段，或者它会代表下游收件人尝试缓冲响应。注意，HTTP/1.1 没有对分片响应的大小做任何限制，因此这些客户端应该确保可以缓冲整个响应。
2. 如果正在测试的传输编码是`TE`字段中列出的传输编码之一，那么它是可接受的。除非它附有q值为0.（如第3.9节所定义，q值为0表示“不可接受”。）
3. 如果可以接受多个传输编码，那么可接受的具有最高的非零值q值的传输编码是首选。 “chunked”传输编码的q值始终为1。
...
`Upgrade`通用首部字段允许客户端指定它支持的或者它希望使用的附加通信协议，如果服务器发现它适合切换协议。服务器必须使用`Upgrade`首部字段在一个 101（Switching Protocols）响应来表示将要切换到哪种协议。
...
`Upgrade`首部字段旨在提供从 HTTP / 1.1 转换到其他一些协议的简单机制，可能是其他不兼容的协议。它通过允许客户端宣称其使用其他协议，比如具有较高主版本号的HTTP版本，的愿望来这样做，即使当前请求已使用 HTTP / 1.1。 这通过允许客户端启动使用更常用的协议得请求，同时向服务器表明它想要使用“更好”协议（如果可用）（其中“更好”由服务器确定，可能根据方法的性质和/或要求的资源）来简化不兼容协议之间的困难转换。

`Upgrade`首部字段仅适用于在现有传输层连接上切换应用层协议。不能用于永久协议更改，服务器对它的接受和使用是可选的。协议更改后应用层通信的功能和性质完全取决于所选择的新协议，虽然改变协议后的第一个动作必须是对初始协议的包含Upgrade头字段的HTTP请求响应。
`Upgrade`首部字段仅适用于即时连接。因此，每当HTTP / 1.1中存在`Upgrade`时，`Upgrade`关键字必须是在`Connection`首部字段（第14.10节）中提供。
`Upgrade`头字段不能用于指示切换到不同连接上的协议。为了那个目的，使用301，302，303 或 305 重定向响应更合适。
该规范仅定义了超文本传输协议族使用的协议名称“HTTP”，如由3.1节的HTTP版本规则和本规范的未来更新定义。任何符号都可以用作协议名称；但是，仅当客户端和服务器将名称与同一个协议相关联时，它才是有用的协议名称。

## 14.43 User-Agent

`User-Agent`请求首部字段包含有关请求源头用户代理的信息。这是用于统计目的，协议违规追踪，或者自动识别用户代理以定制响应来避免特定用户代理限制。用户代理应该在请求中包含此字段。该字段可以包含多个产品标识或者商标，关于用户代理包含的所有主要部分的产品标识等。按照惯例，这些商标按照重要程度排列。
...
`Vary`字段值表示完全确定的请求首部字段集合，当响应是新鲜的，是否缓存被允许可以直接使用该响应直接回复后续的额请求而不需要再次校验。对不能缓存的响应或者已经过时的响应，`Vary`字段值建议用户代理有关选择资源的条件。`Vary`字段的`*`值表示缓存不能根据后续请求的请求首部字段确定该响应是否是合适的表示形式。参考 13.6 章节了解缓存使用`Vary`字段。

````
Vary		= "Vary" ":" ( "*" | 1#field-name )
````

HTTP / 1.1服务器应该包括一个`Vary`首部字段，其中包含任何可缓存的响应，该响应受服务器驱动的协商的约束。这样做允许缓存正确解释该资源上的未来请求并通知用户代理，了解该资源是否存在协商。服务器不可缓存的响应可以包含`Vary`首部字段，受服务器驱动的协商约束，因为这可能为用户代理提供有关响应在响应时响应变化的维度的有用信息。

由一个字段名称列表组成的`Vary`字段值表示基于选择算法为响应选择的表示，该算法在选择最合适的表示时仅考虑列出的请求标题字段值。缓存可以假定将来的请求在响应新鲜的持续时间内将使用相同的选择，列出的字段名称的值相同。

给出的字段名称不限于本规范定义的标准请求首部字段集。领域名称不区分大小写。

`Vary`字段值`*`表示未指定的参数不限于请求首部字段（例如，客户端的网络地址），在响应表示的选择中发挥作用。`*`值绝不可以由代理服务器生成，它可能只由初始服务器生成。

## 14.45 Via

`Via`通用首部字段必须被网关或者代理使用，用来表示请求路径上用户代理与服务器之间的中间协议和消息接收者，以及在响应路径上初始服务器到客户端之间的。它类似于 RFC 822 中的 `Received` 字段，目的是用来追踪消息转发、避免请求环路，以及表明请求/响应链路上的所有消息发送者的协议接受能力。
...
`received-protocol`表示服务器或者客户端沿着请求/响应链路的每一段接收到的消息的协议版本。该接收协议版本会被附加到`Via`字段值后面，当消息被转发时，以便所有通信参与者都能看到上游应用的协议接受能力。

`protocol-name`是可选的，当且仅当它是“HTTP”时。`received-by`字段通常是主机和随后转发消息的收件人服务器或客户端的可选端口号。但是，如果真实的主机地址被认为是敏感信息，可以用假名替换。如果没有给出端口，它可以被假定为`received-protocol`的默认端口。

多个`Via`字段值表示每个已经转发消息的代理或者网关。每个通信参与者都必须将自身的信息附加到字段值之后，这样做的结果就是该字段的值按照后续转发应用的顺序排列。

评论可以被用在`Via`字段中来标识代理或者网关软件，类似于`User-Agent`和`Server`首部字段。不过，`Via`字段中的所有评论都是可选的，可以被任何参与者在转发消息之前删除。

比如，一个请求消息可以被从一个 HTTP/1.0 用户代理发出，发送给一个内部代理名为“fred"，它使用 HTTP/1.1 协议将请求转发给一个 `nowhere.com` 上的公开代理，它会通过将请求转发到 `www.ics.uci.edu` 上的初始服务器来完成请求处理。由 `www.ics.uci.edu` 接收到的请求将携带如下 `Via` 首部字段：

````
Via: 1.0 fred, 1.1 nowhere.com (Apache/1.1)
````

通过网络防火墙用作门户的代理和网关默认情况下不应转发名称和防火墙区域内的主机端口。只有在显式启用时才应传播此信息。如果不启用，防火墙后面的任何主机的`received-by`应该被适当的假名取代.

对于有很严格的安全要求的机构，为了严密隐藏内部网络结构，代理可以将具有相同`received-protocol`值的`Via`首部字段条目的有序子序列排序到单个这样的条目中。比如：
...
应用程序不应该组合多个条目，除非它们都在同一组织控制下同时主机已经被假名取代。应用程序不得组合具有不同的`received-protocol`值的条目。

## 14.46 Warning

`Warning`通用首部字段用来携带关于消息的状态或者转换附加信息，这些附加信息不会在消息中反映出来。这些信息通常用来警告来自作用于消息中的实体主体的缓存操作或者转换导致的语义透明性缺失的可能性风险。
...
一个响应可以包含超过一个`Warning`首部字段。

警告文本`warn-text`应该是一种自然语言和字符集，最有可能被收到该响应的人类用户理解。该决定可以基于任何可用的知识，例如
缓存或用户，请求中的`Accept-Language`字段，响应中的`Content-Language`字段等。默认语言为英语，默认字符集为ISO-8859-1。
如果使用ISO-8859-1以外的字符集，则必须使用在RFC 2047 [14]中描述的方法对`warn-text`警告文本进行编码。

警告首部字段通常可以应用于任何消息，但某些特定的警告代码`warn-codes`是特定与缓存并且只能应用于响应消息的。新警告首部字段应该在任何现有首部字段之后添加。缓存绝不能删除它收到的任何消息中的`Warning`首部字段。但是，如果一个缓存成功验证缓存条目，它应该删除以前附加到它的任何`Warning`首部字段，除特定警告代码指定外的条目。它必须然后添加收到的验证回复中的所有`Warning`首部字段。换句话说，`Warning`首部字段是附加到最新的相关响应上的。

当多个`Warning`首部字段被附加到一个响应，用户代理应该将它们通知用户，按照它们出现在响应中的顺序。如果全部通知不可能，则用户代理应该遵循以下启发式规则：

* 在响应中尽早出现的警告优先于响应中稍后出现的警告。
* 用户首选字符集中的警告优先于其他字符集中的警告但具有相同的警告代码`warn-codes`和警告代理人`warn-agents`。

生成多个警告标题的系统应该在考虑此用户代理行为的情况下对它们进行排序。

有关警告的缓存行为的要求在第13.1.2节中说明。

这是当前定义的警告代码`warn-codes`的列表，每个警告代码都有一个推荐的英文警告文本`warn-text`，以及一个
其含义的描述。

110 响应已过时

​	当返回的响应已经过时的情况下该响应中必须包含

111 再验证失败

​	如果缓存返回陈旧的响应，则必须包含在响应中，因为无法访问服务器导致尝试重新验证响应失败。

112 断开连接操作

​	如果缓存有意与网络其他部分断开连接一段时间，响应中必须包含。

113 启发超时

​	如果缓存启发式地选择了大于24小时的新鲜度生存时间同时响应年龄大于24小时，则必须包含在响应中。

119 杂项警告

​	警告文本可以包括要呈现给人类用户或记录的任意信息。一个系统收到此警告除了向用户显示警告外，不得采取任何自动操作。

214 转换应用

​	必须由中间缓存或代理添加，如果它应用任何改变响应内容编码的转换（在`Content-Encoding`首部字段中指定）或改变响应`media-type`（在`Content-Type`首部字段中指定）的转换，或改变响应的实体主体的转换，除非此警告代码已出现在响应中。

299 杂项持久化警告

​	警告文本可以包括要呈现给人类用户或记录的任意信息。一个系统收到此警告除了向用户显示警告外，不得采取任何自动操作。

如果实现发送带有一个或多个版本为HTTP / 1.0或更低版本的`Warning`首部字段的消息，那么发件人必须在每个`warning-value`中包含一个与响应中的日期相匹配的`warn-date`。
如果实现接收到带有警告值的消息，该警告值包含警告日期，并且该警告与响应中的日期值不同，则在存储，转发或使用消息之前必须从该警告值中删除该警告值。 （这可以防止单纯缓存警告首部字段造成的不良后果。）如果因为这个原因删除了所有`warning-values`，则必须同样删除`Warning`首部字段。
