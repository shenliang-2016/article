...
RFC 1806 [35]，从中衍生出经常被实现的````Content-Disposition````（参见第19.5.1节）首部字段，有许多非常严重的安全问题考虑因素。 ````Content-Disposition````不是 HTTP 标准的一部分，但由于它已被广泛实现，我们正在记录它的使用以及它对实现者的风险。 有关详细信息，请参阅RFC 2183 [49]（更新自 RFC 1806）。
